<template>
  <div v-if="selectedPart" class="part" :class="position">
    <img :src="selectedPart.src" :title="selectedPart.name" @click="showInfo()" />
    <button @click="selectPreviousPart()" class="prev-selector"></button>
    <button @click="selectNextPart()" class="next-selector"></button>
    <span v-pin:position.right.top class="sale" v-show="selectedPart.onSale">Sale!</span>
  </div>
</template>
...
function getPreviousValidIndex(index, length) {
  let deprecatedIndex = index + length - 1;
  deprecatedIndex %= length;
